...
```
问：把 64×64 图像压缩成 32 个离散 token，会丢失什么？
答：视觉细节——小物体位置、精细纹理、微妙变化
...
问：有没有生成模型可以保留这些细节？
答：扩散模型！DALL-E、Stable Diffusion 的成功证明了这一点
```

---

# VAE vs Diffusion：生成方式的本质区别
...
```
问：世界模型需要预测 15 步，每步扩散需要 20 次去噪，意味着？
答：计算量爆炸！

  Dreamer: 15 步 × 1 次前向 = 15 次网络调用
  DIAMOND: 15 步 × 20 次去噪 = 300 次网络调用

问：那 DIAMOND 为什么还能在 Atari 上赢？
答：预测质量的提升弥补了速度的损失
    在视觉细节重要的游戏上，准比快更重要
```
...
**实际应用**：可能是混合——关键场景用 Diffusion，常规情况用 Dreamer
